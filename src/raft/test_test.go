...
// TODO: 将 RaftElectionTimeout 修改位 1 秒，测试 100 次会有 2 次无法选主成功 (超过了测试用例的检测时常被判定为没有 成功选主！)
...
func TestBackup2B(t *testing.T) {
	...
	// 2-51
	source1 := make([]int, 50)
	for i, _ := range source1 {
		source1[i] = i + 2
	}
	for i := 0; i < 50; i++ {
		cfg.rafts[leader1].Start(source1[i])
	}
	...
	// 102-151
	for i := 0; i < 50; i++ {
		cfg.rafts[leader2].Start(source1[i] + 100)
	}
...
func TestFigure82C(t *testing.T) {
	...
	for iters := 0; iters < 1000; iters++ {
		...
		for i := 0; i < servers; i++ {
			if cfg.rafts[i] != nil {
				_, _, ok := cfg.rafts[i].Start(seqInt())
				if ok {
					leader = i
				}
			}
...
func TestFigure8Unreliable2C(t *testing.T) {
	...
	for iters := 0; iters < 1000; iters++ {
		...
		leader := -1
		for i := 0; i < servers; i++ {
			_, _, ok := cfg.rafts[i].Start(seqInt() % 10000)
...
func internalChurn(t *testing.T, unreliable bool) {
	...
	lastIndex := cfg.one(seqInt(), servers)
...
//
func TestLeftSearch(t *testing.T) {
	s := make([]Entry, 0)
	s = append(s, Entry{})
	for i := 1; 33 > i; i++ {
		s = append(s, Entry{Index: i})
	}
	fmt.Printf("log: %v\n", s)
	fmt.Printf("pos: %d, value of this pos: %d\n", leftSearch(s, 32), s[leftSearch(s, 32)])
	fmt.Printf("  ... Passed\n")
}
