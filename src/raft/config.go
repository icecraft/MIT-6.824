		...
		for m := range applyCh {
			...
			if m.UseSnapshot {
				...
				if m.Index > 1 && prevok == false {
					err_msg = fmt.Sprintf("server %v apply out of order %v, command: %v", i, m.Index, m.Command)
				}
			} else {
...
func (cfg *config) one(cmd int, expectedServers int) int {
			...
			for time.Since(t1).Seconds() < 2 {
				...
				if nd > 0 && nd >= expectedServers {
					...
					if cmd2, ok := cmd1.(int); ok && cmd2 == cmd {
						...
						// fmt.Printf("cmd2: %d, cmd: %d\n", cmd2, cmd)
					}
				}

				// fmt.Printf("index: %d, nd: %d, expecedServers: %d, logs: %v\n", index, nd, expectedServers, cfg.logs)
	...
	logL := make(map[int]int, len(cfg.logs))
	for i, v := range cfg.logs {
		logL[i] = len(v)
	}

	fmt.Printf("expecedServers: %d, logs: %v\n", expectedServers, logL)
	Dlog("expecedServers: %d, logs: %v\n", expectedServers, logL)
